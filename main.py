...
def consume_scroll_steps(accumulator, step_size):
    if accumulator >= step_size:
        steps = int(accumulator // step_size)
    elif accumulator <= -step_size:
        steps = -int((-accumulator) // step_size)
    else:
        steps = 0

    return steps, accumulator - steps * step_size
...
def main():
    ...
    scroll_smoother = PointSmoother(SMOOTHING_POINTS)
    ...
    try:
        while True:
            ...
            if tracker.available and results and results.multi_hand_landmarks:
                ...
                scroll_point = tracker.get_scroll_control_point(hand_landmarks, frame_width, frame_height)
                ...
                is_scroll_gesture = tracker.is_scroll_gesture(hand_landmarks)
                ...
                if index_point is not None:
                    ...
                    if mouse.available and mouse_mapper is not None and not is_scroll_gesture:
                ...
                if scroll_point is not None:
                    smoothed_scroll_point = scroll_smoother.smooth(scroll_point)
                    cv2.circle(frame, scroll_point, CONTROL_POINT_RADIUS, STATUS_TEXT_COLOR, 1)
                else:
                    scroll_smoother.reset()
                ...
                if is_left_click_gesture:
                    ...
                    last_scroll_y = None
                    scroll_accumulator = 0.0
                elif is_scroll_gesture and not pinch_gesture_active and not drag_active:
                    if not scroll_gesture_active:
                        scroll_gesture_active = True
                        last_scroll_y = (smoothed_scroll_point or scroll_point)[1] if (smoothed_scroll_point or scroll_point) else None
                        scroll_accumulator = 0.0
                    elif (smoothed_scroll_point or scroll_point) is not None and last_scroll_y is not None:
                        current_scroll_y = (smoothed_scroll_point or scroll_point)[1]
                        scroll_accumulator += last_scroll_y - current_scroll_y
                        scroll_steps, scroll_accumulator = consume_scroll_steps(scroll_accumulator, SCROLL_STEP_PIXELS)
                        if scroll_steps != 0 and mouse.available:
                            mouse.scroll_vertical(scroll_steps)
                            status_text = f"Scroll {scroll_steps:+d}"
                        last_scroll_y = current_scroll_y
                elif scroll_gesture_active:
                    scroll_gesture_active = False
                    last_scroll_y = None
                    scroll_accumulator = 0.0
            ...
            if smoothed_scroll_point is not None:
                cv2.putText(
                    frame,
                    f"Scroll point: {smoothed_scroll_point[0]}, {smoothed_scroll_point[1]}",
                    (20, 360),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.7,
                    STATUS_TEXT_COLOR,
